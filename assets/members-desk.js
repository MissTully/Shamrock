...
'<div class="hub-fb-members">' +
    ...
    list.forEach(function (event) {
      ...
      html += '<div class="hub-event-row">' + thumb + '<div class="hub-event-copy"><b>' + esc(event.name) + '</b>' +
        ...
        (readOnly ? "" : '<div class="hub-appr-btns">' +
          ...
          '<button class="btn" type="button" data-event-checkins="' + eid + '">Who checked in</button>' +
        '</div>') +
  ...
  async function loadEventStudio(client) {
    ...
    card.innerHTML =
      ...
      '<div class="app-body">' +
      '<p style="font-size:16px;color:var(--muted);margin:0 0 12px;">Tap <b>Edit event</b> on any published row to change address, start/end, or registration close. How door check-in works: <b>save the event</b>, tap <b>Door check-in QR</b> (projector at the door), and members scan it while signed in. That marks them attended. Tap <b>Who checked in</b> (or Reports → Event report) to see the list. Saving stores a draft and does not publish.</p>' +
